...
// Used only by the mobile-only hero (components/sections/Hero.jsx, ≤768px —
// see app/globals.css "MOBILE HOMEPAGE REDESIGN"). Deliberately its own
// short set rather than reusing `stats`/`benefits` above: this row is a
// tight three-item "what it's like to work with me" strip, not the
// desktop's fuller 4-stat trust bar or 6-item benefits grid.
export const heroTrustPills = [
  { icon: "Zap", label: "Fast Communication" },
  { icon: "Shield", label: "Reliable & On-Time" },
  { icon: "Globe", label: "Clients in Australia, USA, Germany & more" },
];

// Floating "years" callout over the mobile hero photo — a looser framing
// ("learning & building") than the checkmark row's freelancing-specific
// claim, so it's kept separate rather than reusing that copy.
export const heroYearsBadge = { value: "5+", lines: ["Years of", "Learning & Building"] };

// Doodle-style word stack next to the years badge, mobile hero only.
export const heroDoodleWords = ["Ideas", "Websites", "Growth"];
