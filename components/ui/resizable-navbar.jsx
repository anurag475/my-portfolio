  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        // Pushed as transparent as the design wants: at this tint the links
        // (text-neutral-900) measure ~3.1:1 over the hero's darkest patch —
        // below WCAG AA's 4.5:1 for this text size (bg-white/50 was the AA
        // floor). Kept legible in practice by the blur (above) plus the
        // font-weight/size, but going more transparent than this, or
        // dropping the blur, needs a legibility re-check.
        "relative z-[60] mx-auto hidden w-full max-w-7xl grid-cols-[auto_1fr_auto] items-center self-start rounded-full border border-black/5 bg-white/35 px-4 py-2 lg:grid",
        className
      )}
    >
      {children}
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        // dark, so a fully transparent bar was unreadable before scrolling.
        // Pushed as transparent as the design wants — see the matching note
        // on the desktop NavBody above for the measured contrast tradeoff.
        // `mobile-nav-shell` is a plain, unstyled-by-default hook: the
        // ≤768px "MOBILE HOMEPAGE REDESIGN" block in app/globals.css reskins
        // it dark/glass there only, leaving this Tailwind-driven light-glass
        // look untouched for tablet widths (769–1023px, still this same
        // component) and doing nothing at all above the lg breakpoint,
        // where NavBody (not this component) renders instead.
        "mobile-nav-shell relative z-50 mx-auto flex w-full max-w-[calc(100vw-1.5rem)] flex-col items-center justify-between border border-black/5 bg-white/40 py-2 lg:hidden",
        className
      )}
    >
      {children}
...
export function MobileNavHeader({ children, className }) {
  return <div className={cn("mobile-nav-header flex w-full flex-row items-center justify-between", className)}>{children}</div>;
}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -8 }}
          className={cn(
            "mobile-nav-menu absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-white px-4 py-8 shadow-[0_0_24px_rgba(34,_42,_53,_0.06),_0_1px_1px_rgba(0,_0,_0,_0.05),_0_0_0_1px_rgba(34,_42,_53,_0.04),_0_0_4px_rgba(34,_42,_53,_0.08),_0_16px_68px_rgba(47,_48,_55,_0.05),_0_1px_0_rgba(255,_255,_255,_0.1)_inset]",
            className
          )}
        >
          {children}
...
export function MobileNavToggle({ isOpen, onClick }) {
  return (
    <button
      ...
      className="mobile-nav-toggle p-1 text-neutral-900"
    >
...
export function NavbarLogo({ href = "/", avatar, name, subtitle }) {
  return (
    <a href={href} className="relative z-20 mr-4 flex items-center gap-2 px-2 py-1 text-sm font-normal">
      {avatar}
      <span>
        <span className="nav-logo-name font-semibold text-neutral-900">{name}</span>
        {/* Rendered only where a caller passes `subtitle` (the mobile
            header — see components/SiteNav.jsx); stays `display:none` by
            default (app/globals.css) and is only revealed, styled, at
            ≤768px, so it can never appear on desktop or tablet. */}
        {subtitle && <span className="nav-logo-subtitle">{subtitle}</span>}
      </span>
    </a>
  );
}
